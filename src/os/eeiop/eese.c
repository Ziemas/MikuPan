...
// Used for UI sound effects
int SeStartFix(int se_no, u_short fin_spd, u_short vol_max, u_short pitch,
               u_char menu)
{
    SE_REQ_TMP_STR tmp;
    int vpos;
...
int SeStartFixFlame(int se_no, u_short flame, u_short vol_max, u_short pitch,
                    u_char menu)
{
    u_short fin_spd;
...
int SeStartFixV(int se_no, u_short fin_spd, u_short vol_max, u_short pitch,
                u_char voice_no)
{
    SE_REQ_TMP_STR tmp;
    int vpos;
...
int SeStartPos(int se_no, float *pos, u_short fin_spd, u_short vol_max,
               u_short pitch, u_char room_id)
{
    SE_REQ_TMP_STR tmp;
    int vpos;
...
int SeStartPosFlame(int se_no, float *pos, u_short flame, u_short vol_max,
                    u_short pitch, u_char room_id)
{
    u_short fin_spd;
...
int SeStartPut(int se_no, float *pos, u_short fin_spd, u_short vol_max,
               u_short pitch, u_char room_id)
{
    SE_REQ_TMP_STR tmp;
    int vpos;
...
int SeStartPutFlame(int se_no, float *pos, u_short flame, u_short vol_max,
                    u_short pitch, u_char room_id)
{
    u_short fin_spd;
...
int SeStartGhost(int se_no, int ewrk_no, u_short fin_spd, u_short vol_max,
                 u_short pitch, u_char gno)
{
    SE_REQ_TMP_STR tmp;
    int vpos;
    ENE_WRK *ework;
...
int SeStartGhostFlame(int se_no, int ewrk_no, u_short flame, u_short vol_max,
                      u_short pitch, u_char gno)
{
    u_short fin_spd;
    ...
    switch (tmp_str->mode)
    {
        ...
        case 4:
            ...
            rot_oc = SeCmdGetAngle(
                SgAtan2f(swp->spos[0] - camera.p[0],
                         swp->spos[2] - camera.p[2]),
                SgAtan2f(camera.i[0] - camera.p[0], camera.i[2] - camera.p[2]));
            swp->pan = SeCmdGetPan(rot_oc);
            swp->vol_rate = SeCmdGetVol(GetDist(GetDistV(swp->spos, camera.p),
                                                swp->spos[1] - camera.p[1]),
                                        swp->vol_rate);
...
int SeStartPosEventFlame(u_char room_id, u_char req_pos, float *pos,
...
int SeStop(int voice_num)
{
    if (voice_num >= 0)
    {
        SetIopCmdSm(IC_SE_STOP, voice_num, 0, 0);
    }
...
void SeSetMVol(u_short vol)
{
    ...
    SetIopCmdSm(IC_SE_MVOL, (iop_vol * 0x3fff) / 0xfff, 0, 0);
}
...
void SeSetSteMono(u_char mono)
{
    if (mono != 0)
    {
        SetIopCmdSm(IC_SE_STE_MONO, 1, 0, 0);
    }
    else
    {
        SetIopCmdSm(IC_SE_STE_MONO, 0, 0, 0);
    }
}

// Used for ingame sound effects
...
char SeGetGhostPos(u_int se_file_no, u_char type)
{
    ...
    for (i = 0; i < 3; i++)
    {
        if (se_ctrl.ghost_no[i] == se_file_no
...
char SeGetAutoGhostPos(u_int se_file_no)
{
    ...
    for (i = 0; i < 4; i++)
    {
        if (se_ctrl.jidou_no[i] == se_file_no
...
void SeStartVpCmn(int se_no, int vpos, float *pos, float *mb, int fin_spd,
                  u_short vol_max)
{
    SE_WRK *swp;
    int mode;
    ...
    SetIopCmdLg(IC_SE_PLAY, vpos, swp->se_p, swp->pitch, swp->pan,
                swp->vol_rate, 0, 0);
}
